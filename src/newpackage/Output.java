 ...
 */
public class Output {
     public BigDecimal SumResult; //{ get; set; }         
     public int MulResult; //{ get; set; }         
     public BigDecimal[] SortedInputs; //{ get; set; } 
     
     Output (Input vx) { //Конструктор класса от первоначального объекта
      ...
      try {
	   ...
	   //NodeList nList = document.getElementsByTagName("K"); // Переходим к ноде с коэфф.
         ...
         //       if (nodeX.hasChildNodes()) { //Если имеет дочерние ноды
                ...
                for(int i=0; i<x1.size(); i++){ SortedInputs[i]=x1.get(i); }
          //  }
    ...
    if(serialize.equalsIgnoreCase("json")){ //Разбор json
        ...
        MulResult=rootObject.get("MulResult").getAsInt();
         ...
         for(int i=0; i<list_s.size(); i++) { SortedInputs[i]=BigDecimal.valueOf(list_s.get(i)); }
         
        System.out.println("SumResult= "+SumResult+"  MulResult= "+MulResult+"  SortedInputs= "+SortedInputs.toString()); // print "Hi World!"*/
    }
    }
/*      boolean eq (Output v3){ 
          boolean f1=true;
          if (this.MulResult!=v3.MulResult){ return false; }
          if(!this.SumResult.equals(v3.SumResult)){return false;}
          if(this.SortedInputs.length!=v3.SortedInputs.length){return false;}
          for(int i=0; i<this.SortedInputs.length; i++){
              if(!this.SortedInputs[i].equals(v3.SortedInputs[i])) {return false;}
          }
          return true;
     }
*/
     String Respect (String serialize) throws TransformerConfigurationException, TransformerException {
      ...
      if(serialize.equalsIgnoreCase("json"))  { //throws IOException
        ...
        rootObject.addProperty("SumResult", SumResult); // rootObject.addProperty("message", "Hi");
        
        rootObject.addProperty("MulResult", MulResult); // rootObject.addProperty("message", "Hi");
        
        Gson gson = new Gson();
        String json = gson.toJson(SortedInputs);
        rootObject.addProperty("SortedInputs", json );

        respect = new String (gson.toJson(rootObject)); //String json = gson.toJson(rootObject);
        
        System.out.println(respect);                     // print "{"message":"Hi","place":{"name":"World!"}}" 
      }
      return respect;
     }
}
