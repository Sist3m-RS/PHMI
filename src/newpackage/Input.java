...
public class Input {
    ...
    public BigDecimal Sums[]; // { get; set; }
    public int Muls[]; //{ get; set; }
    
    Input () { // Конструктор для рандомной генерации значений
        Random x1= new Random();
        K=x1.nextInt(11)+1;
        int n1=x1.nextInt(3)+2;
        int n2=x1.nextInt(3)+2;
        Sums=new BigDecimal [n1];
        this.Muls=new int [n2];
        for(int i=0; i<n1; i++){ Sums[i]=BigDecimal.valueOf(x1.nextInt(10)); } //for(int i=0; i<n1; i++){ Sums[i]=BigDecimal.valueOf(x1.nextDouble()); }
        for(int i=0; i<n2; i++){ Muls[i]=x1.nextInt(10); }
    }
    Input(int k1, BigDecimal S1[], int [] M1){ // Конструктор для создания объекта из данных-членов
        K=k1;
        // Добавить try-catch
        if (S1.length>0){
        this.Sums=new BigDecimal [S1.length];
        for(int i=0; i<S1.length; i++){
        Sums[i]=S1[i];
        }
        
       // System.arraycopy(S1, 0, Sums, 0, S1.length);
        } else { JOptionPane.showMessageDialog(null, "Размер массива сумм нулевой !"); }
        
        if (M1.length>0){   
        this.Muls=new int [M1.length];
        for(int i=0; i<M1.length; i++){
        Muls[i]=M1[i];
        }
       // System.arraycopy(M1, 0, Muls, 0, M1.length);
        } else { JOptionPane.showMessageDialog(null, "Размер массива произведений нулевой !"); }
    }
    Input(String serialize, String raw) throws IOException, SAXException, ParserConfigurationException { // Конструктор для создания объекта из строки xml\json
    if(serialize.equalsIgnoreCase("xml")){ // Разбор xml
      try {
           ...
           //  Document document = builder.parse(raw); 
           // Document document = builder.parse(new InputSource(new ByteArrayInputStream(raw.getBytes("utf-8")))); 
            Document document = builder.parse(new InputSource(new StringReader(raw))); // ЗДЕСЬ ОШИБКА БЫЛА !!!! ПИЗДЕЦ КАКАЯ !  
            
            // Получаем корневой элемент
            Node root = document.getDocumentElement();
        //  JOptionPane.showMessageDialog(null,"Корень xml= "+document.getDocumentElement().getNodeName());
	   ...
	   //NodeList nList = document.getElementsByTagName("K"); // Переходим к ноде с коэфф.
           ...
           //  if (nodeX.hasChildNodes()) { //Если имеет дочерние ноды
            ...
            for(int i=0; i<x1.size(); i++){ Sums[i]=x1.get(i); }
            this.Muls=new int [x2.size()];
            for(int i=0; i<x2.size(); i++){ Muls[i]=Integer.parseInt(x2.get(i)); }
        //   }
    ...
    if(serialize.equalsIgnoreCase("json")){ //Разбор json
        ...
        K=rootObject.get("K").getAsInt();
         ...
         for(int i=0; i<list_s.size(); i++) { Sums[i]=BigDecimal.valueOf(list_s.get(i)); }
    ...
    String Request (String serialize) throws TransformerConfigurationException, TransformerException{ //, String request
        ...
        if(serialize.equalsIgnoreCase("json"))  { //throws IOException
        
        JsonObject rootObject = new JsonObject();
        rootObject.addProperty("K", Integer.toString(K)); // rootObject.addProperty("message", "Hi");
        ...
        request = new String (gson.toJson(rootObject)); //String json = gson.toJson(rootObject);
        
        System.out.println(request);                     // print "{"message":"Hi","place":{"name":"World!"}}" 
        }
        return request;
    }
}
